...
public class GestoreParcheggio implements Parcheggio
{
   ...
   public GestoreParcheggio(BackupDB db) throws SQLException {
	   ...
	   //ParcheggioSimulato.parcheggio = this.db.getBackup();
		...
		//	entrante.release();		bisogna rivedere la simulazione (troppa alternanza)
   ...
   public int paga(Ticket ticket, String IBAN, String pswd){
	   
	   Ticket T = find(ticket);
	   //verifica del pagamento
	    if( ! payChecker.pagamento(payChecker.prezzoTicket(T.getReceiveTime().toString()) , IBAN, pswd ) ) {
   ...
   **/
   ...
   private void init() {
	   // Inizialmente il parcheggio è vuoto.
	      ...
	      //rilevante solo per la simulazione
